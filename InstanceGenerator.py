...
def createGraph(type, nbTunnels, nbunnelEndNodes, nbNeighbours, path):
    ...
    tunnels_generator(graph, nbTunnels, nbunnelEndNodes, nbNeighbours, type, path) 
...
def tunnels_generator(G, nbTunnels, nbunnelEndNodes, nbNeighbours, type, path, min_bw = 100, max_bw = 200):
    ...
    for tunnel in range(nbTunnels): 
        ...
        # If type is Type0 the source or the destination must be 0
        if src != 0 and dst != 0 and type == "Type0":
            dst = 0
